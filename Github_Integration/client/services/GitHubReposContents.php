...
class GitHubReposContents extends GitHubService
{
	 ...
	 * @param $msg	string	Required. The commit message.
	...
	public function createFile($owner, $repo, $path, $msg, $content, $branch='master')
	{
		...
		return $this->client->request("/repos/$owner/$repo/contents/$path", 'PUT', $data, 201, 'GitHubCommit');
	}
	...
	public function updateFile($owner, $repo, $path, $msg, $content, $sha, $branch='master')
	{
		...
		return $this->client->request("/repos/$owner/$repo/contents/$path", 'PUT', $data, 200, 'GitHubCommit');
	}
	 ...
	 * @return GitHubCommit
	 */
	public function deleteFile($owner, $repo, $path, $msg, $sha, $branch='master')
	{
		$data = array();
		$data['message'] = $msg;
		$data['branch'] = $branch;
		...
		return $this->client->request("/repos/$owner/$repo/contents/$path", 'DELETE', $data, 200, 'GitHubCommit');
	}
